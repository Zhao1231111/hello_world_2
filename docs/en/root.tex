...
% 期刊版本使用 IEEEtran journal 模板；正文内容迁移自 IROS/root.tex。
...
% 数据集、序列和代表帧名称统一使用正文 Times 斜体，避免被 caption 小型大写环境改成缺失字体形状。
\newcommand{\datasetname}[1]{\mbox{\normalfont\itshape #1}}
% 压缩公式上下间距，使迁移后的英文稿更接近期刊双栏排版密度。
\AtBeginDocument{
...
\title{LIV-Surfel: Feed-Forward Surfel Regression for LiDAR-Inertial-Visual Incremental 2DGS Mapping}

\author{Puwei Zhao$^{\dagger}$, Yiduo Zhou$^{\dagger}$, Yiming Fan, Tong Shi, and Kun Qian$^{*}$,~\IEEEmembership{Member,~IEEE}%
\thanks{$\dagger$ Puwei Zhao and Yiduo Zhou contributed equally to this work.}%
\thanks{$*$ Corresponding author: Kun Qian.}
\thanks{Puwei Zhao, Yiduo Zhou, Yiming Fan, Tong Shi, and Kun Qian are with the School of Automation, Southeast University, Nanjing, China (e-mail: kqian@seu.edu.cn).}}
