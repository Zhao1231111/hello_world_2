...
/**
 * @brief SH 到 RGB 颜色转换的反向传播（设备函数）
 * 
 * 计算损失函数相对于球谐(SH)系数和高斯中心位置的梯度（因为视角方向依赖中心位置）。
 *
 * @param idx 高斯点的索引
 * @param deg 球谐函数的阶数
 * @param max_coeffs 最大 SH 系数数量
 * @param means 高斯 3D 中心位置
 * @param campos 摄像机位置
 * @param shs SH 系数数据的指针
 * @param clamped 标记 RGB 分量是否在前向传播时被钳制
 * @param dL_dcolor 损失相对于输出颜色的梯度 (来自上层)
 * @param dL_dmeans [输出] 累积计算出的相对于 3D 位置的梯度
 * @param dL_dshs [输出] 计算出的相对于 SH 系数的梯度
 */
...
renderCUDA(
	...
	const float* __restrict__ colors,              // 颜色数据 (RGB/SH)
	const float* __restrict__ depths,              // 每个高斯点的深度
	const float* __restrict__ final_Ts,            // 前向传播保存的最终透射率
	const uint32_t* __restrict__ n_contrib,        // 前向传播保存的贡献者数量
	const float* __restrict__ dL_dpixels,          // [梯度输入] 相对于像素颜色的梯度
	const float* __restrict__ dL_depths,           // [梯度输入] 相对于深度的梯度 (2DGS 特有)
	float * __restrict__ dL_dtransMat,             // [输出] 相对于变换矩阵 T 的梯度
	float3* __restrict__ dL_dmean2D,               // [输出] 相对于 2D 位置的梯度
	float* __restrict__ dL_dnormal3D,              // [输出] 相对于 3D 法向的梯度
	float* __restrict__ dL_dopacity,               // [输出] 相对于不透明度的梯度
	float* __restrict__ dL_dcolors)                // [输出] 相对于颜色(RGB)的梯度
{
...
/**
 * @brief 计算变换矩阵 T 和包围盒的高级导数 (2DGS核心)
 * 
 * 将损失相对于矩阵 T 的梯度进一步传播到原始的 3D 参数：
 * 位置 (means)、旋转 (rotations)、缩放 (scales)。
 * 该过程对应 3DGS 的雅可比矩阵链式法则在 2DGS 形式下的推导。
 */
...
/**
 * @brief 反向预处理内核：将梯度流回原始参数
 */
template<int C>
__global__ void preprocessCUDA(
	int P, int D, int M,
	const float3* means3D,            // 原始 3D 位置
	const float* transMats,           // 前向计算出的变换矩阵 T
	const int* radii,                 // 投影半径
	const float* shs,                 // 球谐系数
	const bool* clamped,              // 颜色裁剪标记
	const glm::vec2* scales,          // 2D 缩放
	const glm::vec4* rotations,       // 旋转
	const float scale_modifier,       // 缩放因子
	const float* viewmatrix,          // 视图矩阵
	const float* projmatrix,          // 投影矩阵
	const float focal_x,              // 焦距
	const float focal_y,
	const float tan_fovx,             // FOV
	const float tan_fovy,
	const glm::vec3* campos,          // 相机中心
	float* dL_dtransMats,             // [梯度输入/输出] 相对于矩阵 T 的梯度
	const float* dL_dnormal3Ds,       // [梯度输入] 相对于法向的梯度
	float* dL_dcolors,                // [梯度输入/输出] 相对于颜色的梯度
	float* dL_dshs,                   // [输出] 相对于 SH 的梯度
	float3* dL_dmean2Ds,              // [输出] 相对于 2D 中心(NDC)的梯度
	glm::vec3* dL_dmean3Ds,           // [输出] 相对于 3D 中心的梯度
	glm::vec2* dL_dscales,            // [输出] 相对于缩放的梯度
	glm::vec4* dL_drots)              // [输出] 相对于旋转的梯度
{
